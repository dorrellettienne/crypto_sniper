...
from src.live.live_pilot_service import (
    ...
    resolve_live_pilot_launch_authorization_packet_approval_token_latest_state,
    build_live_pilot_launch_authorization_packet_approval_token_audit_summary,
    ...
    list_live_pilot_launch_authorization_chain_of_chain_approval_token_revocations,
    revoke_live_pilot_launch_authorization_chain_of_chain_approval_token,
    list_live_pilot_launch_authorization_super_chain_approval_token_revocations,
    revoke_live_pilot_launch_authorization_super_chain_approval_token,
    resolve_live_pilot_launch_authorization_chain_approval_token_latest_state,
    build_live_pilot_launch_authorization_chain_approval_token_audit_summary,
    ...
    build_live_pilot_launch_authorization_super_chain_freshness_summary,
...
def test_ticket_consumption_log_and_guard_replay_protection(tmp_path):
    ticket = build_live_pilot_promotion_ticket(
        operator_id="main_user",
        approval_action="approve_live_test",
        prelive_go_no_go_report={"status": "go", "failed_required_checks": []},
    ...
    log_path = tmp_path / "ticket_consumptions.jsonl"
    row = consume_live_pilot_promotion_ticket(
        consumption_log_jsonl_path=str(log_path),
        ticket=ticket,
        reason="live_launch_guard_allow",
    )
    assert row["ticket_id"] == ticket["ticket_id"]
    rows = list_live_pilot_promotion_ticket_consumptions(str(log_path))
    assert len(rows) == 1
    assert rows[0]["event_type"] == "live_pilot_promotion_ticket_consumed"
    ...
    guard = evaluate_live_launch_guard(
        ...
        require_bundle_pass=True,
        require_operator_ticket=True,
        require_unused_ticket=True,
        consumed_tickets=rows,
    )
...
def test_launch_authorization_packet_approval_token_and_guard_checks(tmp_path):
    now_ms = int(time.time() * 1000)
    prelive = {"status": "go", "bundle_verification_status": "pass", "generated_unix_ms": now_ms}
    intent = build_live_pilot_launch_intent_manifest(
        mode="live_auto_tiny_one_trade",
    ...
    assert token["authorization_packet_fingerprint_sha256"] == packet["packet_fingerprint_sha256"]
    md_path = tmp_path / "packet_approval_token.md"
    write_live_pilot_launch_authorization_packet_approval_token(token, str(md_path))
    assert "Authorization Packet Approval Token" in md_path.read_text(encoding="utf-8")
    ...
    assert guard_ok["status"] == "allow"
    assert guard_ok["authorization_packet_approval_token_id"] == token["token_id"]

    bad_token = dict(token)
    bad_token["authorization_packet_fingerprint_sha256"] = "bad_fp"
    guard_bad = evaluate_live_launch_guard(
        ...
        launch_intent_manifest=intent,
        launch_authorization_packet=packet,
        launch_authorization_packet_approval_token=bad_token,
        require_launch_authorization_packet=True,
        require_launch_authorization_packet_approval_token=True,
        max_launch_authorization_packet_age_seconds=3600.0,
        max_launch_authorization_packet_approval_token_age_seconds=3600.0,
...
def test_launch_authorization_packet_approval_token_audit_and_chain_report(tmp_path):
    now_ms = int(time.time() * 1000)
    packet = {
        "status": "authorized",
        "ticket_id": "t1",
        "intent_id": "i1",
        "packet_fingerprint_sha256": "packet_fp",
    }
    token = build_live_pilot_launch_authorization_packet_approval_token(
        launch_authorization_packet=packet,
        operator_id="main_user",
    ...
    revoked_rows = [
        {
            ...
            "token_fingerprint_sha256": token["token_fingerprint_sha256"],
            "authorization_packet_fingerprint_sha256": token["authorization_packet_fingerprint_sha256"],
            "operator_id": "main_user",
            "approval_action": token["approval_action"],
            "reason": "operator_cancelled",
            "reason_class": "operator",
            "severity": "warning",
            "ts_unix_ms": now_ms,
    ...
    audit = build_live_pilot_launch_authorization_packet_approval_token_audit_summary(
        approval_token=token,
        revoked_approval_tokens=revoked_rows,
    )
    assert audit["token_state"]["revoked"] is True
    md_audit = tmp_path / "packet_approval_token_audit.md"
    write_live_pilot_launch_authorization_packet_approval_token_audit_summary(audit, str(md_audit))
    assert "Approval Token Audit" in md_audit.read_text(encoding="utf-8")

    chain = build_live_pilot_launch_authorization_chain_report(
        launch_authorization_packet=packet,
        launch_authorization_packet_approval_token=token,
        launch_authorization_packet_approval_token_audit_summary=audit,
        promotion_ticket_revocation_audit_summary={"ticket_state": {"effective_revoked": False}},
        live_launch_guard_report={"status": "block", "required_failed_checks": ["authorization_packet_approval_token_unrevoked"]},
        launch_authorization_freshness_envelope={"status": "pass"},
    )
    assert chain["status"] == "blocked"
    assert "packet_approval_token_not_revoked" in chain["failed_required_checks"]
    md_chain = tmp_path / "launch_authorization_chain.md"
    write_live_pilot_launch_authorization_chain_report(chain, str(md_chain))
    assert "Authorization Chain Report" in md_chain.read_text(encoding="utf-8")


def test_launch_authorization_chain_freshness_summary_and_guard_requirement(tmp_path):
    now_ms = int(time.time() * 1000)
    chain = build_live_pilot_launch_authorization_chain_report(
    ...
    chain["generated_unix_ms"] = now_ms
    chain["chain_report_fingerprint_sha256"] = __import__("hashlib").sha256(
        json.dumps({k: v for k, v in chain.items() if k != "chain_report_fingerprint_sha256"}, sort_keys=True).encode("utf-8")
    ).hexdigest()
    fresh = {"status": "pass", "stale_sources": ["launch_intent_manifest"]}
    summary = build_live_pilot_launch_authorization_chain_freshness_summary(
        launch_authorization_chain_report=chain,
        launch_authorization_freshness_envelope=fresh,
        max_chain_report_age_seconds=60.0,
    )
    assert summary["status"] == "pass"
    md_path = tmp_path / "launch_auth_chain_freshness.md"
    write_live_pilot_launch_authorization_chain_freshness_summary(summary, str(md_path))
    assert "Chain Freshness Summary" in md_path.read_text(encoding="utf-8")
    ...
    guard_allow = evaluate_live_launch_guard(
        ...
        max_launch_authorization_chain_report_age_seconds=60.0,
    )
    assert guard_allow["status"] == "allow"
    assert guard_allow["authorization_chain_report_status"] == "ready"

    stale_chain = dict(chain)
    stale_chain["generated_unix_ms"] = now_ms - 10_000_000
    guard_block = evaluate_live_launch_guard(
        adapter_config={"live_send_network_enabled": True},
        enable_live_auto_submit_window=True,
        launch_authorization_chain_report=stale_chain,
        require_launch_authorization_chain_report=True,
        max_launch_authorization_chain_report_age_seconds=1.0,
    )
    assert guard_block["status"] == "block"
    assert "authorization_chain_report_fresh_enough" in guard_block["required_failed_checks"]


def test_launch_authorization_chain_fingerprint_binding_and_chain_approval_token(tmp_path):
    chain = build_live_pilot_launch_authorization_chain_report(
        launch_authorization_packet={"status": "authorized", "packet_fingerprint_sha256": "p1"},
        launch_authorization_packet_approval_token={"token_id": "pat1", "authorization_packet_fingerprint_sha256": "p1"},
        launch_authorization_packet_approval_token_audit_summary={"token_state": {"revoked": False}},
    ...
    assert chain["chain_report_fingerprint_sha256"]
    assert chain["binding"]["packet_fingerprint_sha256"] == "p1"
    md_chain = tmp_path / "chain.md"
    write_live_pilot_launch_authorization_chain_report(chain, str(md_chain))
    assert "chain_report_fingerprint_sha256" in md_chain.read_text(encoding="utf-8")
    ...
    assert chain_token["chain_report_fingerprint_sha256"] == chain["chain_report_fingerprint_sha256"]
    md_tok = tmp_path / "chain_token.md"
    write_live_pilot_launch_authorization_chain_approval_token(chain_token, str(md_tok))
    assert "Chain Approval Token" in md_tok.read_text(encoding="utf-8")
    ...
    assert guard_ok["status"] == "allow"
    assert guard_ok["authorization_chain_approval_token_id"] == chain_token["token_id"]

    bad_chain = dict(chain)
    bad_chain["summary"] = dict(chain["summary"])
    bad_chain["summary"]["packet_status"] = "tampered"
    guard_bad_chain = evaluate_live_launch_guard(
        adapter_config={"live_send_network_enabled": True},
        enable_live_auto_submit_window=True,
        launch_authorization_chain_report=bad_chain,
        require_launch_authorization_chain_report=True,
        max_launch_authorization_chain_report_age_seconds=3600.0,
    )
...
def test_launch_authorization_chain_approval_token_audit_and_chain_of_chain_report(tmp_path):
    now_ms = int(time.time() * 1000)
    chain = build_live_pilot_launch_authorization_chain_report(
        launch_authorization_packet={"status": "authorized", "packet_fingerprint_sha256": "p1"},
        launch_authorization_packet_approval_token={"token_id": "pat1", "authorization_packet_fingerprint_sha256": "p1"},
    ...
    revoked_rows = [
        {
            ...
            "token_id": chain_token["token_id"],
            "token_fingerprint_sha256": chain_token["token_fingerprint_sha256"],
            "chain_report_fingerprint_sha256": chain_token["chain_report_fingerprint_sha256"],
            "operator_id": "main_user",
            "approval_action": chain_token["approval_action"],
            "reason": "operator_cancelled",
            "reason_class": "operator",
            "severity": "warning",
            "ts_unix_ms": now_ms,
    ...
    audit = build_live_pilot_launch_authorization_chain_approval_token_audit_summary(
        approval_token=chain_token,
        revoked_approval_tokens=revoked_rows,
    )
    assert audit["token_state"]["revoked"] is True
    md_audit = tmp_path / "chain_approval_token_audit.md"
    write_live_pilot_launch_authorization_chain_approval_token_audit_summary(audit, str(md_audit))
    assert "Chain Approval Token Audit" in md_audit.read_text(encoding="utf-8")

    chain_of_chain = build_live_pilot_launch_authorization_chain_of_chain_report(
        launch_authorization_chain_report=chain,
        launch_authorization_chain_approval_token=chain_token,
        launch_authorization_chain_approval_token_audit_summary=audit,
        live_launch_guard_report={"status": "block", "required_failed_checks": ["authorization_chain_approval_token_unrevoked"]},
    )
    assert chain_of_chain["status"] == "blocked"
    assert "chain_approval_token_not_revoked" in chain_of_chain["failed_required_checks"]
...
def test_launch_authorization_chain_of_chain_freshness_summary_and_guard_requirement(tmp_path):
    now_ms = int(time.time() * 1000)
    chain_of_chain = build_live_pilot_launch_authorization_chain_of_chain_report(
        launch_authorization_chain_report={
            "status": "ready",
            "chain_report_fingerprint_sha256": "cfp1",
        },
        launch_authorization_chain_approval_token={
            "token_id": "lca_1",
            "chain_report_fingerprint_sha256": "cfp1",
        },
    ...
    chain_of_chain["status"] = "ready"
    chain_of_chain["generated_unix_ms"] = now_ms
    chain_of_chain["chain_of_chain_report_fingerprint_sha256"] = __import__("hashlib").sha256(
        json.dumps({k: v for k, v in chain_of_chain.items() if k != "chain_of_chain_report_fingerprint_sha256"}, sort_keys=True).encode("utf-8")
    ).hexdigest()
...
def test_launch_authorization_chain_of_chain_fingerprint_and_guard_binding_checks():
    now_ms = int(time.time() * 1000)
    chain = build_live_pilot_launch_authorization_chain_report(
        launch_authorization_packet={"status": "authorized", "packet_fingerprint_sha256": "p1"},
        launch_authorization_packet_approval_token={"token_id": "pat1", "authorization_packet_fingerprint_sha256": "p1"},
    ...
    assert chain2["chain_of_chain_report_fingerprint_sha256"]
    ...
    assert guard_ok["status"] == "allow"
    assert guard_ok["authorization_chain_of_chain_report_fingerprint_valid"] is True

    tampered_chain2 = dict(chain2)
    tampered_chain2["summary"] = dict(chain2["summary"])
    tampered_chain2["summary"]["guard_status"] = "block"
    guard_bad_fp = evaluate_live_launch_guard(
        adapter_config={"live_send_network_enabled": True},
        enable_live_auto_submit_window=True,
        launch_authorization_chain_report=chain,
        launch_authorization_chain_approval_token=chain_token,
        launch_authorization_chain_of_chain_report=tampered_chain2,
        require_launch_authorization_chain_of_chain_report=True,
        max_launch_authorization_chain_of_chain_report_age_seconds=3600.0,
    )
    assert guard_bad_fp["status"] == "block"
    assert "authorization_chain_of_chain_report_fingerprint_valid" in guard_bad_fp["required_failed_checks"]

    other_chain_token = dict(chain_token)
    other_chain_token["token_id"] = "different_chain_token"
    guard_bad_binding = evaluate_live_launch_guard(
        adapter_config={"live_send_network_enabled": True},
        enable_live_auto_submit_window=True,
        launch_authorization_chain_report=chain,
        launch_authorization_chain_approval_token=other_chain_token,
        launch_authorization_chain_of_chain_report=chain2,
        require_launch_authorization_chain_of_chain_report=True,
        require_launch_authorization_chain_of_chain_report_binding=True,
        max_launch_authorization_chain_of_chain_report_age_seconds=3600.0,
...
def test_launch_authorization_chain_of_chain_approval_token_revocation_and_guard_unrevoked_check(tmp_path):
    now_ms = int(time.time() * 1000)
    chain = build_live_pilot_launch_authorization_chain_report(
        launch_authorization_packet={"status": "authorized", "packet_fingerprint_sha256": "p1"},
        launch_authorization_packet_approval_token={"token_id": "pat1", "authorization_packet_fingerprint_sha256": "p1"},
    ...
    log_path = tmp_path / "chain_of_chain_approval_token_revocations.jsonl"
    row = revoke_live_pilot_launch_authorization_chain_of_chain_approval_token(
        revocation_log_jsonl_path=str(log_path),
        approval_token=token2,
        operator_id="main_user",
        reason="operator_cancelled",
    )
    assert row["event_type"] == "live_pilot_launch_authorization_chain_of_chain_approval_token_revoked"
    rows = list_live_pilot_launch_authorization_chain_of_chain_approval_token_revocations(str(log_path))
    assert len(rows) == 1
    ...
    guard = evaluate_live_launch_guard(
        ...
        require_launch_authorization_chain_of_chain_report=True,
        require_launch_authorization_chain_of_chain_approval_token=True,
        require_unrevoked_launch_authorization_chain_of_chain_approval_token=True,
        max_launch_authorization_chain_of_chain_report_age_seconds=3600.0,
        max_launch_authorization_chain_of_chain_approval_token_age_seconds=3600.0,
        revoked_launch_authorization_chain_of_chain_approval_tokens=rows,
    )
    assert guard["status"] == "block"
...
def test_launch_authorization_super_chain_freshness_summary_and_guard_requirement(tmp_path):
    now_ms = int(time.time() * 1000)
    super_chain = build_live_pilot_launch_authorization_super_chain_report(
        launch_authorization_chain_of_chain_report={
            "status": "ready",
            "chain_of_chain_report_fingerprint_sha256": "c2fp1",
        },
        launch_authorization_chain_of_chain_approval_token={
            "token_id": "lc2a_1",
            "chain_of_chain_report_fingerprint_sha256": "c2fp1",
        },
        launch_authorization_chain_of_chain_approval_token_audit_summary={"token_state": {"revoked": False}},
        live_launch_guard_report={"status": "allow", "required_failed_checks": [], "generated_unix_ms": now_ms},
    )
    super_chain["status"] = "ready"
    super_chain["generated_unix_ms"] = now_ms
    super_chain["super_chain_report_fingerprint_sha256"] = __import__("hashlib").sha256(
        json.dumps({k: v for k, v in super_chain.items() if k != "super_chain_report_fingerprint_sha256"}, sort_keys=True).encode("utf-8")
    ).hexdigest()
    summary = build_live_pilot_launch_authorization_super_chain_freshness_summary(
        launch_authorization_super_chain_report=super_chain,
        launch_authorization_chain_of_chain_freshness_summary={"status": "pass"},
        max_super_chain_report_age_seconds=60.0,
    )
    assert summary["status"] == "pass"
    md_path = tmp_path / "super_chain_freshness.md"
    write_live_pilot_launch_authorization_super_chain_freshness_summary(summary, str(md_path))
    assert "Super-Chain Freshness Summary" in md_path.read_text(encoding="utf-8")

    guard_allow = evaluate_live_launch_guard(
        adapter_config={"live_send_network_enabled": True},
        enable_live_auto_submit_window=True,
        launch_authorization_super_chain_report=super_chain,
        require_launch_authorization_super_chain_report=True,
        max_launch_authorization_super_chain_report_age_seconds=60.0,
    )
    assert guard_allow["status"] == "allow"
    assert guard_allow["authorization_super_chain_report_status"] == "ready"

    stale_super_chain = dict(super_chain)
    stale_super_chain["generated_unix_ms"] = now_ms - 10_000_000
    guard_block = evaluate_live_launch_guard(
        adapter_config={"live_send_network_enabled": True},
        enable_live_auto_submit_window=True,
        launch_authorization_super_chain_report=stale_super_chain,
        require_launch_authorization_super_chain_report=True,
        max_launch_authorization_super_chain_report_age_seconds=1.0,
    )
    assert guard_block["status"] == "block"
    assert "authorization_super_chain_report_fresh_enough" in guard_block["required_failed_checks"]


def test_launch_authorization_super_chain_fingerprint_and_guard_binding_checks():
    now_ms = int(time.time() * 1000)
    chain2 = build_live_pilot_launch_authorization_chain_of_chain_report(
        launch_authorization_chain_report={"status": "ready", "chain_report_fingerprint_sha256": "cfp"},
        launch_authorization_chain_approval_token={"token_id": "lca_1", "chain_report_fingerprint_sha256": "cfp"},
    ...
    assert super_chain["super_chain_report_fingerprint_sha256"]
    ...
    guard_ok = evaluate_live_launch_guard(
        ...
        launch_authorization_super_chain_report=super_chain,
        require_launch_authorization_super_chain_report=True,
        require_launch_authorization_super_chain_report_binding=True,
        max_launch_authorization_super_chain_report_age_seconds=3600.0,
    )
    assert guard_ok["status"] == "allow"
    assert guard_ok["authorization_super_chain_report_fingerprint_valid"] is True

    tampered_super = dict(super_chain)
    tampered_super["summary"] = dict(super_chain["summary"])
    tampered_super["summary"]["guard_status"] = "block"
    guard_bad_fp = evaluate_live_launch_guard(
        adapter_config={"live_send_network_enabled": True},
        enable_live_auto_submit_window=True,
        launch_authorization_super_chain_report=tampered_super,
        require_launch_authorization_super_chain_report=True,
        max_launch_authorization_super_chain_report_age_seconds=3600.0,
    )
    assert guard_bad_fp["status"] == "block"
    assert "authorization_super_chain_report_fingerprint_valid" in guard_bad_fp["required_failed_checks"]

    other_token2 = dict(token2)
    other_token2["token_id"] = "different_chain2_token"
    guard_bad_binding = evaluate_live_launch_guard(
        adapter_config={"live_send_network_enabled": True},
        enable_live_auto_submit_window=True,
        launch_authorization_chain_of_chain_report=chain2,
        launch_authorization_chain_of_chain_approval_token=other_token2,
        launch_authorization_super_chain_report=super_chain,
        require_launch_authorization_super_chain_report=True,
        require_launch_authorization_super_chain_report_binding=True,
        max_launch_authorization_super_chain_report_age_seconds=3600.0,
    ...
    assert guard_bad_binding["status"] == "block"
    assert "authorization_super_chain_report_bound_to_current_inputs" in guard_bad_binding["required_failed_checks"]


def test_launch_authorization_super_chain_approval_token_and_guard_checks(tmp_path):
    now_ms = int(time.time() * 1000)
    chain2 = build_live_pilot_launch_authorization_chain_of_chain_report(
        launch_authorization_chain_report={"status": "ready", "chain_report_fingerprint_sha256": "cfp"},
        launch_authorization_chain_approval_token={"token_id": "lca_1", "chain_report_fingerprint_sha256": "cfp"},
    ...
    super_token = build_live_pilot_launch_authorization_super_chain_approval_token(
        launch_authorization_super_chain_report=super_chain,
        operator_id="main_user",
        approval_action="approve_live_launch_super_chain",
        expires_in_seconds=900,
    )
    assert super_token["super_chain_report_fingerprint_sha256"] == super_chain["super_chain_report_fingerprint_sha256"]
    md_tok = tmp_path / "super_chain_token.md"
    write_live_pilot_launch_authorization_super_chain_approval_token(super_token, str(md_tok))
    assert "Super-Chain Approval Token" in md_tok.read_text(encoding="utf-8")

    guard_ok = evaluate_live_launch_guard(
        adapter_config={"live_send_network_enabled": True},
        enable_live_auto_submit_window=True,
        launch_authorization_super_chain_report=super_chain,
        launch_authorization_super_chain_approval_token=super_token,
        require_launch_authorization_super_chain_report=True,
        require_launch_authorization_super_chain_approval_token=True,
        max_launch_authorization_super_chain_report_age_seconds=3600.0,
        max_launch_authorization_super_chain_approval_token_age_seconds=3600.0,
    )
    assert guard_ok["status"] == "allow"
    assert guard_ok["authorization_super_chain_approval_token_id"] == super_token["token_id"]

    bad_super_token = dict(super_token)
    bad_super_token["super_chain_report_fingerprint_sha256"] = "bad_fp"
    guard_bad = evaluate_live_launch_guard(
        adapter_config={"live_send_network_enabled": True},
        enable_live_auto_submit_window=True,
        launch_authorization_super_chain_report=super_chain,
        launch_authorization_super_chain_approval_token=bad_super_token,
        require_launch_authorization_super_chain_approval_token=True,
        max_launch_authorization_super_chain_approval_token_age_seconds=3600.0,
    )
    assert guard_bad["status"] == "block"
    assert "authorization_super_chain_approval_token_matches_super_chain_fingerprint" in guard_bad["required_failed_checks"]


def test_launch_authorization_super_chain_approval_token_revocation_and_guard_unrevoked_check(tmp_path):
    now_ms = int(time.time() * 1000)
    chain2 = build_live_pilot_launch_authorization_chain_of_chain_report(
        launch_authorization_chain_report={"status": "ready", "chain_report_fingerprint_sha256": "cfp"},
        launch_authorization_chain_approval_token={"token_id": "lca_1", "chain_report_fingerprint_sha256": "cfp"},
    ...
    log_path = tmp_path / "super_chain_approval_token_revocations.jsonl"
    row = revoke_live_pilot_launch_authorization_super_chain_approval_token(
        revocation_log_jsonl_path=str(log_path),
        approval_token=super_token,
        operator_id="main_user",
        reason="operator_cancelled",
    )
    assert row["event_type"] == "live_pilot_launch_authorization_super_chain_approval_token_revoked"
    rows = list_live_pilot_launch_authorization_super_chain_approval_token_revocations(str(log_path))
    assert len(rows) == 1
    ...
    guard = evaluate_live_launch_guard(
        ...
        require_launch_authorization_super_chain_report=True,
        require_launch_authorization_super_chain_approval_token=True,
        require_unrevoked_launch_authorization_super_chain_approval_token=True,
        max_launch_authorization_super_chain_report_age_seconds=3600.0,
        max_launch_authorization_super_chain_approval_token_age_seconds=3600.0,
        revoked_launch_authorization_super_chain_approval_tokens=rows,
    )
    assert guard["status"] == "block"
    assert guard["authorization_super_chain_approval_token_revoked"] is True
    assert "authorization_super_chain_approval_token_unrevoked" in guard["required_failed_checks"]
