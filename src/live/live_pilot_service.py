...
def list_live_pilot_launch_authorization_chain_approval_token_revocations(revocation_log_jsonl_path: str) -> list[dict[str, Any]]:
    return [
        dict(x)
        for x in _read_jsonl_rows(str(revocation_log_jsonl_path or ""))
        if isinstance(x, dict) and str(x.get("event_type") or "") == "live_pilot_launch_authorization_chain_approval_token_revoked"
    ]


def list_live_pilot_launch_authorization_chain_of_chain_approval_token_revocations(revocation_log_jsonl_path: str) -> list[dict[str, Any]]:
    return [
        dict(x)
        for x in _read_jsonl_rows(str(revocation_log_jsonl_path or ""))
        if isinstance(x, dict) and str(x.get("event_type") or "") == "live_pilot_launch_authorization_chain_of_chain_approval_token_revoked"
    ]


def list_live_pilot_launch_authorization_super_chain_approval_token_revocations(revocation_log_jsonl_path: str) -> list[dict[str, Any]]:
    return [
        dict(x)
        for x in _read_jsonl_rows(str(revocation_log_jsonl_path or ""))
        if isinstance(x, dict) and str(x.get("event_type") or "") == "live_pilot_launch_authorization_super_chain_approval_token_revoked"
    ]
...
def revoke_live_pilot_launch_authorization_chain_approval_token(
    *,
    revocation_log_jsonl_path: str,
    approval_token: dict[str, Any] | None,
    operator_id: str = "",
    ...
    row = {
        "ts_unix_ms": int(time.time() * 1000),
        "event_type": "live_pilot_launch_authorization_chain_approval_token_revoked",
        "token_id": str(tok.get("token_id") or ""),
        "token_fingerprint_sha256": str(tok.get("token_fingerprint_sha256") or ""),
...
def revoke_live_pilot_launch_authorization_chain_of_chain_approval_token(
    *,
    revocation_log_jsonl_path: str,
    approval_token: dict[str, Any] | None,
    operator_id: str = "",
    ...
    row = {
        ...
        "token_id": str(tok.get("token_id") or ""),
        "token_fingerprint_sha256": str(tok.get("token_fingerprint_sha256") or ""),
        "chain_of_chain_report_fingerprint_sha256": str(tok.get("chain_of_chain_report_fingerprint_sha256") or ""),
        "operator_id": str(operator_id or tok.get("operator_id") or ""),
        "approval_action": str(tok.get("approval_action") or ""),
        "reason": reason_text,
        "reason_class": str(reason_meta.get("reason_class") or "other"),
...
def revoke_live_pilot_launch_authorization_super_chain_approval_token(
    *,
    revocation_log_jsonl_path: str,
    approval_token: dict[str, Any] | None,
    operator_id: str = "",
    ...
    row = {
        "ts_unix_ms": int(time.time() * 1000),
        "event_type": "live_pilot_launch_authorization_super_chain_approval_token_revoked",
        "token_id": str(tok.get("token_id") or ""),
        "token_fingerprint_sha256": str(tok.get("token_fingerprint_sha256") or ""),
        "super_chain_report_fingerprint_sha256": str(tok.get("super_chain_report_fingerprint_sha256") or ""),
        "operator_id": str(operator_id or tok.get("operator_id") or ""),
        "approval_action": str(tok.get("approval_action") or ""),
        "reason": reason_text,
        "reason_class": str(reason_meta.get("reason_class") or "other"),
...
def resolve_live_pilot_launch_authorization_chain_of_chain_approval_token_latest_state(
    *,
    approval_token: dict[str, Any] | None = None,
    revoked_approval_tokens: list[dict[str, Any]] | None = None,
) -> dict[str, Any]:
...
def build_live_pilot_launch_authorization_chain_of_chain_approval_token_audit_summary(
    *,
    approval_token: dict[str, Any] | None = None,
    revoked_approval_tokens: list[dict[str, Any]] | None = None,
) -> dict[str, Any]:
...
def write_live_pilot_launch_authorization_chain_of_chain_approval_token_audit_summary(report: dict[str, Any], path_str: str) -> None:
    p = Path(path_str)
    if p.suffix.lower() in {".md", ".markdown"}:
        st = dict(report.get("token_state") or {})
        lines = [
            "# Launch Authorization Chain-of-Chain Approval Token Audit",
            "",
            f"- token_id: `{report.get('token_id', '')}`",
            f"- latest_state: `{st.get('latest_state', '')}`",
            f"- revoked: `{bool(st.get('revoked', False))}`",
...
def evaluate_live_launch_guard(
    *,
    ...
    launch_authorization_chain_of_chain_approval_token: dict[str, Any] | None = None,
    launch_authorization_super_chain_report: dict[str, Any] | None = None,
    launch_authorization_super_chain_approval_token: dict[str, Any] | None = None,
    requested_mode: str = "",
    ...
    require_launch_authorization_chain_of_chain_report_binding: bool = False,
    ...
    require_launch_authorization_super_chain_approval_token: bool = False,
    require_unrevoked_launch_authorization_super_chain_approval_token: bool = False,
    revocation_reason_class_policy_overrides: dict[str, str] | None = None,
    ...
    required_launch_authorization_chain_approval_action: str = "approve_live_launch_chain",
    ...
    max_launch_authorization_chain_of_chain_approval_token_age_seconds: float = 900.0,
    max_launch_authorization_super_chain_report_age_seconds: float = 900.0,
    max_launch_authorization_super_chain_approval_token_age_seconds: float = 900.0,
    consumed_tickets: list[dict[str, Any]] | None = None,
    revoked_tickets: list[dict[str, Any]] | None = None,
    revoked_launch_authorization_packet_approval_tokens: list[dict[str, Any]] | None = None,
    revoked_launch_authorization_chain_approval_tokens: list[dict[str, Any]] | None = None,
    revoked_launch_authorization_chain_of_chain_approval_tokens: list[dict[str, Any]] | None = None,
    revoked_launch_authorization_super_chain_approval_tokens: list[dict[str, Any]] | None = None,
) -> dict[str, Any]:
    cfg = dict(adapter_config or {})
    ...
    auth_chain_of_chain_approval_token = dict(launch_authorization_chain_of_chain_approval_token or {})
    auth_super_chain_report = dict(launch_authorization_super_chain_report or {})
    auth_super_chain_approval_token = dict(launch_authorization_super_chain_approval_token or {})
    ...
    revoked_chain2_token_rows = [dict(x) for x in list(revoked_launch_authorization_chain_of_chain_approval_tokens or []) if isinstance(x, dict)]
    revoked_super_chain_token_rows = [dict(x) for x in list(revoked_launch_authorization_super_chain_approval_tokens or []) if isinstance(x, dict)]
    ...
    auth_chain_generated_ms = _to_int_or_none(auth_chain_report.get("generated_unix_ms"))
    auth_chain_age_ok = False
    if auth_chain_generated_ms is not None:
        auth_chain_age_ok = (now_ms - int(auth_chain_generated_ms)) <= int(max(0.0, float(max_launch_authorization_chain_report_age_seconds)) * 1000.0)
    checks.append(
        {
            "name": "authorization_chain_report_present",
            "required": bool(require_launch_authorization_chain_report),
            "ok": bool(auth_chain_report),
            "actual": bool(auth_chain_report),
        }
    )
    checks.append(
        {
            "name": "authorization_chain_report_status_ready",
            "required": bool(require_launch_authorization_chain_report),
            "ok": str(auth_chain_report.get("status") or "") == "ready",
            "actual": str(auth_chain_report.get("status") or ""),
        }
    )
    checks.append(
        {
            "name": "authorization_chain_report_fresh_enough",
            "required": bool(require_launch_authorization_chain_report),
            "ok": auth_chain_age_ok,
            "actual": (None if auth_chain_generated_ms is None else max(0, now_ms - int(auth_chain_generated_ms))),
        }
    )
    chain_fp_actual = str(auth_chain_report.get("chain_report_fingerprint_sha256") or "")
    chain_fp_expected = ""
    if auth_chain_report:
        chain_fp_expected = hashlib.sha256(
            json.dumps({k: v for k, v in auth_chain_report.items() if k != "chain_report_fingerprint_sha256"}, sort_keys=True).encode("utf-8")
        ).hexdigest()
    checks.append(
        {
            "name": "authorization_chain_report_fingerprint_valid",
            "required": bool(require_launch_authorization_chain_report),
            "ok": (bool(auth_chain_report) and bool(chain_fp_actual) and chain_fp_actual == chain_fp_expected),
            "actual": {"chain_report_fingerprint_sha256": chain_fp_actual, "expected_chain_report_fingerprint_sha256": chain_fp_expected},
        }
    )
    expected_chain_binding = {
        "packet_fingerprint_sha256": str(auth_packet.get("packet_fingerprint_sha256") or ""),
        "packet_approval_token_id": str(auth_packet_approval_token.get("token_id") or ""),
    }
    chain_binding = dict(auth_chain_report.get("binding") or {})
    checks.append(
        {
            "name": "authorization_chain_report_bound_to_current_inputs",
            "required": bool(require_launch_authorization_chain_report),
            "ok": (
                bool(auth_chain_report)
                and (
                    not expected_chain_binding["packet_fingerprint_sha256"]
                    or str(chain_binding.get("packet_fingerprint_sha256") or "") == expected_chain_binding["packet_fingerprint_sha256"]
                )
                and (
                    not expected_chain_binding["packet_approval_token_id"]
                    or str(chain_binding.get("packet_approval_token_id") or "") == expected_chain_binding["packet_approval_token_id"]
                )
            ),
            "actual": {"chain_binding": chain_binding, "expected_binding": expected_chain_binding},
        }
    )
    chain_token_issued_ms = _to_int_or_none(auth_chain_approval_token.get("issued_unix_ms"))
    chain_token_age_ok = False
    if chain_token_issued_ms is not None:
        chain_token_age_ok = (now_ms - int(chain_token_issued_ms)) <= int(max(0.0, float(max_launch_authorization_chain_approval_token_age_seconds)) * 1000.0)
    chain_token_expires_ms = _to_int_or_none(auth_chain_approval_token.get("expires_unix_ms"))
    chain_token_fp_actual = str(auth_chain_approval_token.get("token_fingerprint_sha256") or "")
    chain_token_fp_expected = ""
    if auth_chain_approval_token:
        chain_token_fp_expected = hashlib.sha256(
            json.dumps({k: v for k, v in auth_chain_approval_token.items() if k != "token_fingerprint_sha256"}, sort_keys=True).encode("utf-8")
        ).hexdigest()
    checks.append({"name": "authorization_chain_approval_token_present", "required": bool(require_launch_authorization_chain_approval_token), "ok": bool(auth_chain_approval_token), "actual": bool(auth_chain_approval_token)})
    checks.append(
        {
            "name": "authorization_chain_approval_token_action_matches",
            "required": bool(require_launch_authorization_chain_approval_token),
            "ok": str(auth_chain_approval_token.get("approval_action") or "") == str(required_launch_authorization_chain_approval_action or ""),
            "actual": str(auth_chain_approval_token.get("approval_action") or ""),
        }
    )
    checks.append(
        {
            "name": "authorization_chain_approval_token_not_expired",
            "required": bool(require_launch_authorization_chain_approval_token),
            "ok": (chain_token_expires_ms is not None and int(chain_token_expires_ms) >= now_ms),
            "actual": chain_token_expires_ms,
        }
    )
    checks.append(
        {
            "name": "authorization_chain_approval_token_fresh_enough",
            "required": bool(require_launch_authorization_chain_approval_token),
            "ok": chain_token_age_ok,
            "actual": (None if chain_token_issued_ms is None else max(0, now_ms - int(chain_token_issued_ms))),
        }
    )
    checks.append(
        {
            "name": "authorization_chain_approval_token_fingerprint_valid",
            "required": bool(require_launch_authorization_chain_approval_token),
            "ok": (bool(auth_chain_approval_token) and bool(chain_token_fp_actual) and chain_token_fp_actual == chain_token_fp_expected),
            "actual": {"token_fingerprint_sha256": chain_token_fp_actual, "expected_token_fingerprint_sha256": chain_token_fp_expected},
        }
    )
    checks.append(
        {
            "name": "authorization_chain_approval_token_matches_chain_fingerprint",
            "required": bool(require_launch_authorization_chain_approval_token),
            "ok": (
                bool(auth_chain_approval_token)
                and bool(auth_chain_report)
                and str(auth_chain_approval_token.get("chain_report_fingerprint_sha256") or "") == str(auth_chain_report.get("chain_report_fingerprint_sha256") or "")
            ),
            "actual": {
                "token_chain_report_fingerprint_sha256": str(auth_chain_approval_token.get("chain_report_fingerprint_sha256") or ""),
                "chain_report_fingerprint_sha256": str(auth_chain_report.get("chain_report_fingerprint_sha256") or ""),
            },
        }
    )
    chain_approval_token_id = str(auth_chain_approval_token.get("token_id") or "")
    chain_approval_token_fp = str(auth_chain_approval_token.get("token_fingerprint_sha256") or "")
    chain_approval_token_revoked = any(
        (chain_approval_token_id and str(r.get("token_id") or "") == chain_approval_token_id)
        or (chain_approval_token_fp and str(r.get("token_fingerprint_sha256") or "") == chain_approval_token_fp)
        for r in revoked_chain_token_rows
    )
    checks.append(
        {
            "name": "authorization_chain_approval_token_unrevoked",
            "required": bool(require_launch_authorization_chain_approval_token and require_unrevoked_launch_authorization_chain_approval_token),
            "ok": (not chain_approval_token_revoked),
    ...
    chain2_fp_actual = str(auth_chain_of_chain_report.get("chain_of_chain_report_fingerprint_sha256") or "")
    chain2_fp_expected = ""
    if auth_chain_of_chain_report:
        chain2_fp_expected = hashlib.sha256(
            json.dumps(
                {k: v for k, v in auth_chain_of_chain_report.items() if k != "chain_of_chain_report_fingerprint_sha256"},
                sort_keys=True,
            ).encode("utf-8")
        ).hexdigest()
    checks.append(
        {
            "name": "authorization_chain_of_chain_report_fingerprint_valid",
            "required": bool(require_launch_authorization_chain_of_chain_report),
            "ok": (bool(auth_chain_of_chain_report) and bool(chain2_fp_actual) and chain2_fp_actual == chain2_fp_expected),
            "actual": {
                "chain_of_chain_report_fingerprint_sha256": chain2_fp_actual,
                "expected_chain_of_chain_report_fingerprint_sha256": chain2_fp_expected,
            },
        }
    )
    chain2_binding = dict(auth_chain_of_chain_report.get("binding") or {})
    expected_chain2_binding = {
        "chain_report_fingerprint_sha256": str(auth_chain_report.get("chain_report_fingerprint_sha256") or ""),
        "chain_approval_token_id": str(auth_chain_approval_token.get("token_id") or ""),
    }
    checks.append(
        {
            "name": "authorization_chain_of_chain_report_bound_to_current_inputs",
            "required": bool(require_launch_authorization_chain_of_chain_report and require_launch_authorization_chain_of_chain_report_binding),
            "ok": (
                bool(auth_chain_of_chain_report)
                and (
                    not expected_chain2_binding["chain_report_fingerprint_sha256"]
                    or str(chain2_binding.get("chain_report_fingerprint_sha256") or "") == expected_chain2_binding["chain_report_fingerprint_sha256"]
                )
                and (
                    not expected_chain2_binding["chain_approval_token_id"]
                    or str(chain2_binding.get("chain_approval_token_id") or "") == expected_chain2_binding["chain_approval_token_id"]
                )
            ),
            "actual": {"chain_of_chain_binding": chain2_binding, "expected_binding": expected_chain2_binding},
        }
    )
    chain2_token_issued_ms = _to_int_or_none(auth_chain_of_chain_approval_token.get("issued_unix_ms"))
    chain2_token_age_ok = False
    if chain2_token_issued_ms is not None:
        chain2_token_age_ok = (now_ms - int(chain2_token_issued_ms)) <= int(max(0.0, float(max_launch_authorization_chain_of_chain_approval_token_age_seconds)) * 1000.0)
    chain2_token_expires_ms = _to_int_or_none(auth_chain_of_chain_approval_token.get("expires_unix_ms"))
    chain2_token_fp_actual = str(auth_chain_of_chain_approval_token.get("token_fingerprint_sha256") or "")
    chain2_token_fp_expected = ""
    if auth_chain_of_chain_approval_token:
        chain2_token_fp_expected = hashlib.sha256(
            json.dumps({k: v for k, v in auth_chain_of_chain_approval_token.items() if k != "token_fingerprint_sha256"}, sort_keys=True).encode("utf-8")
        ).hexdigest()
    checks.append(
        {
            "name": "authorization_chain_of_chain_approval_token_present",
            "required": bool(require_launch_authorization_chain_of_chain_approval_token),
            "ok": bool(auth_chain_of_chain_approval_token),
            "actual": bool(auth_chain_of_chain_approval_token),
        }
    )
    checks.append(
        {
            "name": "authorization_chain_of_chain_approval_token_action_matches",
            "required": bool(require_launch_authorization_chain_of_chain_approval_token),
            "ok": str(auth_chain_of_chain_approval_token.get("approval_action") or "") == "approve_live_launch_chain_of_chain",
            "actual": str(auth_chain_of_chain_approval_token.get("approval_action") or ""),
        }
    )
    checks.append(
        {
            "name": "authorization_chain_of_chain_approval_token_not_expired",
            "required": bool(require_launch_authorization_chain_of_chain_approval_token),
            "ok": (chain2_token_expires_ms is not None and int(chain2_token_expires_ms) >= now_ms),
            "actual": chain2_token_expires_ms,
        }
    )
    checks.append(
        {
            "name": "authorization_chain_of_chain_approval_token_fresh_enough",
            "required": bool(require_launch_authorization_chain_of_chain_approval_token),
            "ok": chain2_token_age_ok,
            "actual": (None if chain2_token_issued_ms is None else max(0, now_ms - int(chain2_token_issued_ms))),
        }
    )
    checks.append(
        {
            "name": "authorization_chain_of_chain_approval_token_fingerprint_valid",
            "required": bool(require_launch_authorization_chain_of_chain_approval_token),
            "ok": (bool(auth_chain_of_chain_approval_token) and bool(chain2_token_fp_actual) and chain2_token_fp_actual == chain2_token_fp_expected),
            "actual": {
                "token_fingerprint_sha256": chain2_token_fp_actual,
                "expected_token_fingerprint_sha256": chain2_token_fp_expected,
            },
        }
    )
    checks.append(
        {
            "name": "authorization_chain_of_chain_approval_token_matches_chain_of_chain_fingerprint",
            "required": bool(require_launch_authorization_chain_of_chain_approval_token),
            "ok": (
                bool(auth_chain_of_chain_approval_token)
                and bool(auth_chain_of_chain_report)
                and str(auth_chain_of_chain_approval_token.get("chain_of_chain_report_fingerprint_sha256") or "")
                == str(auth_chain_of_chain_report.get("chain_of_chain_report_fingerprint_sha256") or "")
            ),
            "actual": {
                "token_chain_of_chain_report_fingerprint_sha256": str(auth_chain_of_chain_approval_token.get("chain_of_chain_report_fingerprint_sha256") or ""),
                "chain_of_chain_report_fingerprint_sha256": str(auth_chain_of_chain_report.get("chain_of_chain_report_fingerprint_sha256") or ""),
            },
        }
    )
    chain2_approval_token_id = str(auth_chain_of_chain_approval_token.get("token_id") or "")
    chain2_approval_token_fp = str(auth_chain_of_chain_approval_token.get("token_fingerprint_sha256") or "")
    chain2_approval_token_revoked = any(
        (chain2_approval_token_id and str(r.get("token_id") or "") == chain2_approval_token_id)
        or (chain2_approval_token_fp and str(r.get("token_fingerprint_sha256") or "") == chain2_approval_token_fp)
        for r in revoked_chain2_token_rows
    )
    checks.append(
        {
            "name": "authorization_chain_of_chain_approval_token_unrevoked",
            "required": bool(
                require_launch_authorization_chain_of_chain_approval_token
                and require_unrevoked_launch_authorization_chain_of_chain_approval_token
            ),
            "ok": (not chain2_approval_token_revoked),
            "actual": chain2_approval_token_revoked,
        }
    )
    auth_super_chain_generated_ms = _to_int_or_none(auth_super_chain_report.get("generated_unix_ms"))
    auth_super_chain_age_ok = False
    if auth_super_chain_generated_ms is not None:
        auth_super_chain_age_ok = (now_ms - int(auth_super_chain_generated_ms)) <= int(max(0.0, float(max_launch_authorization_super_chain_report_age_seconds)) * 1000.0)
    checks.append(
        {
            "name": "authorization_super_chain_report_present",
            "required": bool(require_launch_authorization_super_chain_report),
            "ok": bool(auth_super_chain_report),
            "actual": bool(auth_super_chain_report),
        }
    )
    checks.append(
        {
            "name": "authorization_super_chain_report_status_ready",
            "required": bool(require_launch_authorization_super_chain_report),
            "ok": str(auth_super_chain_report.get("status") or "") == "ready",
            "actual": str(auth_super_chain_report.get("status") or ""),
        }
    )
    checks.append(
        {
            "name": "authorization_super_chain_report_fresh_enough",
            "required": bool(require_launch_authorization_super_chain_report),
            "ok": auth_super_chain_age_ok,
    ...
    super_chain_token_issued_ms = _to_int_or_none(auth_super_chain_approval_token.get("issued_unix_ms"))
    super_chain_token_age_ok = False
    if super_chain_token_issued_ms is not None:
        super_chain_token_age_ok = (
            now_ms - int(super_chain_token_issued_ms)
        ) <= int(max(0.0, float(max_launch_authorization_super_chain_approval_token_age_seconds)) * 1000.0)
    super_chain_token_expires_ms = _to_int_or_none(auth_super_chain_approval_token.get("expires_unix_ms"))
    super_chain_token_fp_actual = str(auth_super_chain_approval_token.get("token_fingerprint_sha256") or "")
    super_chain_token_fp_expected = ""
    if auth_super_chain_approval_token:
        super_chain_token_fp_expected = hashlib.sha256(
            json.dumps({k: v for k, v in auth_super_chain_approval_token.items() if k != "token_fingerprint_sha256"}, sort_keys=True).encode("utf-8")
        ).hexdigest()
    checks.append(
        {
            "name": "authorization_super_chain_approval_token_present",
            "required": bool(require_launch_authorization_super_chain_approval_token),
            "ok": bool(auth_super_chain_approval_token),
            "actual": bool(auth_super_chain_approval_token),
        }
    )
    checks.append(
        {
            "name": "authorization_super_chain_approval_token_action_matches",
            "required": bool(require_launch_authorization_super_chain_approval_token),
            "ok": str(auth_super_chain_approval_token.get("approval_action") or "") == "approve_live_launch_super_chain",
            "actual": str(auth_super_chain_approval_token.get("approval_action") or ""),
        }
    )
    checks.append(
        {
            "name": "authorization_super_chain_approval_token_not_expired",
            "required": bool(require_launch_authorization_super_chain_approval_token),
            "ok": (super_chain_token_expires_ms is not None and int(super_chain_token_expires_ms) >= now_ms),
            "actual": super_chain_token_expires_ms,
        }
    )
    checks.append(
        {
            "name": "authorization_super_chain_approval_token_fresh_enough",
            "required": bool(require_launch_authorization_super_chain_approval_token),
            "ok": super_chain_token_age_ok,
            "actual": (None if super_chain_token_issued_ms is None else max(0, now_ms - int(super_chain_token_issued_ms))),
        }
    )
    checks.append(
        {
            "name": "authorization_super_chain_approval_token_fingerprint_valid",
            "required": bool(require_launch_authorization_super_chain_approval_token),
            "ok": (
                bool(auth_super_chain_approval_token)
                and bool(super_chain_token_fp_actual)
                and super_chain_token_fp_actual == super_chain_token_fp_expected
            ),
            "actual": {
                "token_fingerprint_sha256": super_chain_token_fp_actual,
                "expected_token_fingerprint_sha256": super_chain_token_fp_expected,
            },
        }
    )
    checks.append(
        {
            "name": "authorization_super_chain_approval_token_matches_super_chain_fingerprint",
            "required": bool(require_launch_authorization_super_chain_approval_token),
            "ok": (
                bool(auth_super_chain_approval_token)
                and bool(auth_super_chain_report)
                and str(auth_super_chain_approval_token.get("super_chain_report_fingerprint_sha256") or "")
                == str(auth_super_chain_report.get("super_chain_report_fingerprint_sha256") or "")
            ),
            "actual": {
                "token_super_chain_report_fingerprint_sha256": str(auth_super_chain_approval_token.get("super_chain_report_fingerprint_sha256") or ""),
                "super_chain_report_fingerprint_sha256": str(auth_super_chain_report.get("super_chain_report_fingerprint_sha256") or ""),
            },
        }
    )
    super_chain_approval_token_id = str(auth_super_chain_approval_token.get("token_id") or "")
    super_chain_approval_token_fp = str(auth_super_chain_approval_token.get("token_fingerprint_sha256") or "")
    super_chain_approval_token_revoked = any(
        (super_chain_approval_token_id and str(r.get("token_id") or "") == super_chain_approval_token_id)
        or (super_chain_approval_token_fp and str(r.get("token_fingerprint_sha256") or "") == super_chain_approval_token_fp)
        for r in revoked_super_chain_token_rows
    )
    checks.append(
        {
            "name": "authorization_super_chain_approval_token_unrevoked",
            "required": bool(
                require_launch_authorization_super_chain_approval_token
                and require_unrevoked_launch_authorization_super_chain_approval_token
            ),
            "ok": (not super_chain_approval_token_revoked),
            "actual": super_chain_approval_token_revoked,
        }
    )
    ...
    return {
        ...
        "authorization_chain_of_chain_report_fingerprint_valid": bool(
            auth_chain_of_chain_report and chain2_fp_actual and chain2_fp_actual == chain2_fp_expected
        ),
        ...
        "authorization_super_chain_approval_token_id": str(auth_super_chain_approval_token.get("token_id") or ""),
        "authorization_super_chain_approval_token_revoked": super_chain_approval_token_revoked,
...
def build_live_pilot_launch_authorization_chain_report(
    *,
    ...
    out["binding"] = {
        "packet_fingerprint_sha256": str(s.get("packet_fingerprint_sha256", "")) if (s := dict(out.get("summary") or {})) else "",
        "packet_approval_token_id": str(s.get("packet_approval_token_id", "")),
        "packet_approval_token_revoked": bool(s.get("packet_approval_token_revoked", False)),
        "guard_status": str(s.get("guard_status", "")),
        "guard_required_failed_checks": list(s.get("guard_required_failed_checks", []) or []),
        "freshness_status": str(s.get("freshness_status", "")),
    }
    fp_src = json.dumps({k: v for k, v in out.items() if k != "chain_report_fingerprint_sha256"}, sort_keys=True)
    out["chain_report_fingerprint_sha256"] = hashlib.sha256(fp_src.encode("utf-8")).hexdigest()
    return out
...
def write_live_pilot_launch_authorization_chain_report(report: dict[str, Any], path_str: str) -> None:
    ...
    if p.suffix.lower() in {".md", ".markdown"}:
        ...
        lines = [
            ...
            "",
            f"- status: `{report.get('status', '')}`",
            f"- chain_report_fingerprint_sha256: `{report.get('chain_report_fingerprint_sha256', '')}`",
            f"- failed_required_checks: `{', '.join(list(report.get('failed_required_checks', []) or [])) or '-'}`",
...
def build_live_pilot_launch_authorization_chain_approval_token(
    *,
    launch_authorization_chain_report: dict[str, Any] | None = None,
    operator_id: str = "",
    approval_action: str = "approve_live_launch_chain",
    expires_in_seconds: float = 900.0,
) -> dict[str, Any]:
    ...
    payload = {
        ...
        "expires_unix_ms": now_ms + int(max(0.0, float(expires_in_seconds)) * 1000.0),
        "operator_id": str(operator_id or ""),
        "approval_action": str(approval_action or "approve_live_launch_chain"),
        "chain_report_status": str(chain.get("status") or ""),
        "chain_report_fingerprint_sha256": str(chain.get("chain_report_fingerprint_sha256") or ""),
    }
    token_id_seed = json.dumps(payload, sort_keys=True)
    payload["token_id"] = "lca_" + hashlib.sha256(token_id_seed.encode("utf-8")).hexdigest()[:16]
    token_fp_src = json.dumps(payload, sort_keys=True)
    payload["token_fingerprint_sha256"] = hashlib.sha256(token_fp_src.encode("utf-8")).hexdigest()
    return payload


def write_live_pilot_launch_authorization_chain_approval_token(report: dict[str, Any], path_str: str) -> None:
    p = Path(path_str)
    if p.suffix.lower() in {".md", ".markdown"}:
        lines = [
            "# Launch Authorization Chain Approval Token",
            "",
            f"- token_id: `{report.get('token_id', '')}`",
            f"- operator_id: `{report.get('operator_id', '')}`",
            f"- approval_action: `{report.get('approval_action', '')}`",
...
def build_live_pilot_launch_authorization_chain_of_chain_approval_token(
    *,
    launch_authorization_chain_of_chain_report: dict[str, Any] | None = None,
    operator_id: str = "",
    approval_action: str = "approve_live_launch_chain_of_chain",
    expires_in_seconds: float = 900.0,
) -> dict[str, Any]:
    ...
    payload = {
        ...
        "expires_unix_ms": now_ms + int(max(0.0, float(expires_in_seconds)) * 1000.0),
        "operator_id": str(operator_id or ""),
        "approval_action": str(approval_action or "approve_live_launch_chain_of_chain"),
        "chain_of_chain_report_status": str(chain2.get("status") or ""),
        "chain_of_chain_report_fingerprint_sha256": str(chain2.get("chain_of_chain_report_fingerprint_sha256") or ""),
    }
    token_id_seed = json.dumps(payload, sort_keys=True)
    payload["token_id"] = "lc2a_" + hashlib.sha256(token_id_seed.encode("utf-8")).hexdigest()[:16]
    token_fp_src = json.dumps(payload, sort_keys=True)
    payload["token_fingerprint_sha256"] = hashlib.sha256(token_fp_src.encode("utf-8")).hexdigest()
    return payload


def write_live_pilot_launch_authorization_chain_of_chain_approval_token(report: dict[str, Any], path_str: str) -> None:
    p = Path(path_str)
    if p.suffix.lower() in {".md", ".markdown"}:
        lines = [
            "# Launch Authorization Chain-of-Chain Approval Token",
            "",
            f"- token_id: `{report.get('token_id', '')}`",
            f"- operator_id: `{report.get('operator_id', '')}`",
            f"- approval_action: `{report.get('approval_action', '')}`",
...
def build_live_pilot_launch_authorization_chain_freshness_summary(
    *,
    launch_authorization_chain_report: dict[str, Any] | None = None,
    launch_authorization_freshness_envelope: dict[str, Any] | None = None,
    max_chain_report_age_seconds: float = 900.0,
) -> dict[str, Any]:
    chain = dict(launch_authorization_chain_report or {})
    fresh = dict(launch_authorization_freshness_envelope or {})
    now_ms = int(time.time() * 1000)
    chain_generated_ms = _to_int_or_none(chain.get("generated_unix_ms"))
    chain_age_ok = False
    if chain_generated_ms is not None:
        chain_age_ok = (now_ms - int(chain_generated_ms)) <= int(max(0.0, float(max_chain_report_age_seconds)) * 1000.0)
    checks = [
        {"name": "chain_report_present", "required": True, "ok": bool(chain), "actual": bool(chain)},
        {"name": "chain_report_status_ready", "required": True, "ok": str(chain.get("status") or "") == "ready", "actual": str(chain.get("status") or "")},
        {
            "name": "chain_report_fresh_enough",
            "required": True,
            "ok": chain_age_ok,
            "actual": (None if chain_generated_ms is None else max(0, now_ms - int(chain_generated_ms))),
        },
        {
            "name": "chain_freshness_envelope_pass",
            "required": False,
            "ok": str(fresh.get("status") or "") == "pass",
            "actual": str(fresh.get("status") or ""),
        },
    ]
    failed_required = [c["name"] for c in checks if bool(c.get("required", False)) and not bool(c.get("ok", False))]
    stale_sources = list(fresh.get("stale_sources", []) or []) if isinstance(fresh, dict) else []
    return {
        "generated_unix_ms": now_ms,
        "status": ("pass" if not failed_required else "fail"),
        "failed_required_checks": failed_required,
        "chain_report_status": str(chain.get("status") or ""),
        "chain_report_stale_sources": stale_sources,
        "checks": checks,
    }


def write_live_pilot_launch_authorization_chain_freshness_summary(report: dict[str, Any], path_str: str) -> None:
    p = Path(path_str)
    if p.suffix.lower() in {".md", ".markdown"}:
        lines = [
            "# Live Pilot Launch Authorization Chain Freshness Summary",
            "",
            f"- status: `{report.get('status', '')}`",
            f"- chain_report_status: `{report.get('chain_report_status', '')}`",
            f"- failed_required_checks: `{', '.join(list(report.get('failed_required_checks', []) or [])) or '-'}`",
            f"- chain_report_stale_sources: `{', '.join(list(report.get('chain_report_stale_sources', []) or [])) or '-'}`",
            "",
            "## Checks",
            "",
        ]
...
def build_live_pilot_launch_authorization_super_chain_report(
    *,
    launch_authorization_chain_of_chain_report: dict[str, Any] | None = None,
    launch_authorization_chain_of_chain_approval_token: dict[str, Any] | None = None,
    launch_authorization_chain_of_chain_approval_token_audit_summary: dict[str, Any] | None = None,
    live_launch_guard_report: dict[str, Any] | None = None,
) -> dict[str, Any]:
    chain2 = dict(launch_authorization_chain_of_chain_report or {})
    tok2 = dict(launch_authorization_chain_of_chain_approval_token or {})
    tok2_audit = dict(launch_authorization_chain_of_chain_approval_token_audit_summary or {})
    guard = dict(live_launch_guard_report or {})
    checks = [
        {"name": "chain_of_chain_report_present", "required": True, "ok": bool(chain2), "actual": bool(chain2)},
        {"name": "chain_of_chain_report_status_ready", "required": True, "ok": str(chain2.get("status") or "") == "ready", "actual": str(chain2.get("status") or "")},
        {"name": "chain_of_chain_approval_token_present", "required": True, "ok": bool(tok2), "actual": bool(tok2)},
        {
            "name": "chain_of_chain_approval_token_matches_chain_of_chain",
            "required": True,
            "ok": bool(tok2)
            and bool(chain2)
            and str(tok2.get("chain_of_chain_report_fingerprint_sha256") or "") == str(chain2.get("chain_of_chain_report_fingerprint_sha256") or ""),
            "actual": {
                "token_chain_of_chain_report_fingerprint_sha256": str(tok2.get("chain_of_chain_report_fingerprint_sha256") or ""),
                "chain_of_chain_report_fingerprint_sha256": str(chain2.get("chain_of_chain_report_fingerprint_sha256") or ""),
            },
        },
        {"name": "chain_of_chain_approval_token_not_revoked", "required": True, "ok": not bool(((tok2_audit.get("token_state") or {}).get("revoked", False))), "actual": bool(((tok2_audit.get("token_state") or {}).get("revoked", False)))} ,
        {"name": "guard_allow", "required": False, "ok": str(guard.get("status") or "") == "allow", "actual": str(guard.get("status") or "")},
    ]
    failed_required = [c["name"] for c in checks if bool(c.get("required", False)) and not bool(c.get("ok", False))]
    status = "ready" if not failed_required else "blocked"
    out = {
        ...
        "failed_required_checks": failed_required,
        "summary": {
            "chain_of_chain_status": str(chain2.get("status") or ""),
            "chain_of_chain_report_fingerprint_sha256": str(chain2.get("chain_of_chain_report_fingerprint_sha256") or ""),
            "chain_of_chain_approval_token_id": str(tok2.get("token_id") or ""),
            "chain_of_chain_approval_token_revoked": bool(((tok2_audit.get("token_state") or {}).get("revoked", False))),
            "guard_status": str(guard.get("status") or ""),
            "guard_required_failed_checks": list(guard.get("required_failed_checks", []) or []),
        },
        "checks": checks,
    ...
    s = dict(out.get("summary") or {})
    out["binding"] = {
        "chain_of_chain_report_fingerprint_sha256": str(s.get("chain_of_chain_report_fingerprint_sha256", "")),
        "chain_of_chain_approval_token_id": str(s.get("chain_of_chain_approval_token_id", "")),
        "chain_of_chain_approval_token_revoked": bool(s.get("chain_of_chain_approval_token_revoked", False)),
        "guard_status": str(s.get("guard_status", "")),
        "guard_required_failed_checks": list(s.get("guard_required_failed_checks", []) or []),
    }
    fp_src = json.dumps({k: v for k, v in out.items() if k != "super_chain_report_fingerprint_sha256"}, sort_keys=True)
    out["super_chain_report_fingerprint_sha256"] = hashlib.sha256(fp_src.encode("utf-8")).hexdigest()
    return out


def write_live_pilot_launch_authorization_super_chain_report(report: dict[str, Any], path_str: str) -> None:
    p = Path(path_str)
    if p.suffix.lower() in {".md", ".markdown"}:
        s = dict(report.get("summary") or {})
        lines = [
            "# Live Pilot Launch Authorization Super-Chain Report",
            "",
            f"- status: `{report.get('status', '')}`",
            f"- super_chain_report_fingerprint_sha256: `{report.get('super_chain_report_fingerprint_sha256', '')}`",
            f"- failed_required_checks: `{', '.join(list(report.get('failed_required_checks', []) or [])) or '-'}`",
            f"- chain_of_chain_status: `{s.get('chain_of_chain_status', '')}`",
            f"- chain_of_chain_approval_token_id: `{s.get('chain_of_chain_approval_token_id', '')}`",
            f"- chain_of_chain_approval_token_revoked: `{bool(s.get('chain_of_chain_approval_token_revoked', False))}`",
            f"- guard_status: `{s.get('guard_status', '')}`",
            "",
            "## Checks",
            "",
...
def build_live_pilot_launch_authorization_super_chain_approval_token(
    *,
    launch_authorization_super_chain_report: dict[str, Any] | None = None,
    operator_id: str = "",
    approval_action: str = "approve_live_launch_super_chain",
    expires_in_seconds: float = 900.0,
) -> dict[str, Any]:
    super_chain = dict(launch_authorization_super_chain_report or {})
    now_ms = int(time.time() * 1000)
    payload = {
        "issued_unix_ms": now_ms,
        "expires_unix_ms": now_ms + int(max(0.0, float(expires_in_seconds)) * 1000.0),
        "operator_id": str(operator_id or ""),
        "approval_action": str(approval_action or "approve_live_launch_super_chain"),
        "super_chain_report_status": str(super_chain.get("status") or ""),
        "super_chain_report_fingerprint_sha256": str(super_chain.get("super_chain_report_fingerprint_sha256") or ""),
    }
    token_id_seed = json.dumps(payload, sort_keys=True)
    payload["token_id"] = "lsca_" + hashlib.sha256(token_id_seed.encode("utf-8")).hexdigest()[:16]
    token_fp_src = json.dumps(payload, sort_keys=True)
    payload["token_fingerprint_sha256"] = hashlib.sha256(token_fp_src.encode("utf-8")).hexdigest()
    return payload


def write_live_pilot_launch_authorization_super_chain_approval_token(report: dict[str, Any], path_str: str) -> None:
    p = Path(path_str)
    if p.suffix.lower() in {".md", ".markdown"}:
        lines = [
            "# Launch Authorization Super-Chain Approval Token",
            "",
            f"- token_id: `{report.get('token_id', '')}`",
            f"- operator_id: `{report.get('operator_id', '')}`",
            f"- approval_action: `{report.get('approval_action', '')}`",
...
def build_live_pilot_launch_authorization_super_chain_freshness_summary(
    *,
    launch_authorization_super_chain_report: dict[str, Any] | None = None,
    launch_authorization_chain_of_chain_freshness_summary: dict[str, Any] | None = None,
    max_super_chain_report_age_seconds: float = 900.0,
) -> dict[str, Any]:
    super_chain = dict(launch_authorization_super_chain_report or {})
    chain2_fresh = dict(launch_authorization_chain_of_chain_freshness_summary or {})
    now_ms = int(time.time() * 1000)
    generated_ms = _to_int_or_none(super_chain.get("generated_unix_ms"))
    age_ok = False
    if generated_ms is not None:
        age_ok = (now_ms - int(generated_ms)) <= int(max(0.0, float(max_super_chain_report_age_seconds)) * 1000.0)
    checks = [
        {"name": "super_chain_report_present", "required": True, "ok": bool(super_chain), "actual": bool(super_chain)},
        {"name": "super_chain_report_status_ready", "required": True, "ok": str(super_chain.get("status") or "") == "ready", "actual": str(super_chain.get("status") or "")},
        {
            "name": "super_chain_report_fresh_enough",
            "required": True,
            "ok": age_ok,
            "actual": (None if generated_ms is None else max(0, now_ms - int(generated_ms))),
        },
        {
            "name": "chain_of_chain_freshness_summary_pass",
            "required": False,
            "ok": str(chain2_fresh.get("status") or "") == "pass",
            "actual": str(chain2_fresh.get("status") or ""),
        },
    ]
    failed_required = [c["name"] for c in checks if bool(c.get("required", False)) and not bool(c.get("ok", False))]
    return {
        ...
        "failed_required_checks": failed_required,
        "super_chain_report_status": str(super_chain.get("status") or ""),
        "chain_of_chain_report_status": str(((super_chain.get("summary") or {}) if isinstance(super_chain.get("summary"), dict) else {}).get("chain_of_chain_status") or ""),
        "checks": checks,
    }


def write_live_pilot_launch_authorization_super_chain_freshness_summary(report: dict[str, Any], path_str: str) -> None:
    p = Path(path_str)
    if p.suffix.lower() in {".md", ".markdown"}:
        lines = [
            "# Live Pilot Launch Authorization Super-Chain Freshness Summary",
            "",
            f"- status: `{report.get('status', '')}`",
            f"- super_chain_report_status: `{report.get('super_chain_report_status', '')}`",
            f"- chain_of_chain_report_status: `{report.get('chain_of_chain_report_status', '')}`",
            f"- failed_required_checks: `{', '.join(list(report.get('failed_required_checks', []) or [])) or '-'}`",
            "",
            "## Checks",
...
def _main() -> int:
    ...
    p.add_argument("--launch-authorization-chain-of-chain-approval-token-path", default="")
    p.add_argument("--launch-authorization-chain-of-chain-approval-token-audit-report-path", default="")
    p.add_argument("--launch-authorization-super-chain-report-path", default="")
    p.add_argument("--launch-authorization-super-chain-freshness-summary-path", default="")
    p.add_argument("--launch-authorization-super-chain-approval-token-path", default="")
    p.add_argument("--launch-authorization-super-chain-approval-token-revocation-log-jsonl-path", default="")
    p.add_argument("--launch-authorization-super-chain-approval-token-revoke-now", action="store_true")
    p.add_argument("--launch-authorization-super-chain-approval-token-revoke-reason", default="manual_revoke")
    p.add_argument("--launch-authorization-super-chain-approval-token-revoke-only", action="store_true")
    p.add_argument("--launch-authorization-super-chain-approval-action", default="approve_live_launch_super_chain")
    p.add_argument("--launch-authorization-super-chain-approval-expires-seconds", type=float, default=900.0)
    p.add_argument("--launch-authorization-chain-of-chain-approval-token-revocation-log-jsonl-path", default="")
    p.add_argument("--launch-authorization-chain-of-chain-approval-token-revoke-now", action="store_true")
    p.add_argument("--launch-authorization-chain-of-chain-approval-token-revoke-reason", default="manual_revoke")
    p.add_argument("--launch-authorization-chain-of-chain-approval-token-revoke-only", action="store_true")
    p.add_argument("--launch-authorization-chain-of-chain-approval-action", default="approve_live_launch_chain_of_chain")
    p.add_argument("--launch-authorization-chain-of-chain-approval-expires-seconds", type=float, default=900.0)
    ...
    p.add_argument("--live-launch-guard-require-authorization-chain-of-chain-report-binding", action="store_true")
    ...
    p.add_argument("--live-launch-guard-require-authorization-super-chain-approval-token", action="store_true")
    p.add_argument("--live-launch-guard-require-unrevoked-authorization-super-chain-approval-token", action="store_true")
    ...
    p.add_argument("--live-launch-guard-max-authorization-chain-of-chain-approval-token-age-seconds", type=float, default=900.0)
    p.add_argument("--live-launch-guard-max-authorization-super-chain-report-age-seconds", type=float, default=900.0)
    p.add_argument("--live-launch-guard-max-authorization-super-chain-approval-token-age-seconds", type=float, default=900.0)
    ...
    if not args.allow_unsafe_paths:
        ...
        if args.launch_authorization_chain_freshness_summary_path:
        ...
        if args.launch_authorization_chain_of_chain_approval_token_path:
            ensure_dir_within_base(str(Path(args.launch_authorization_chain_of_chain_approval_token_path).parent))
        if args.launch_authorization_chain_of_chain_approval_token_audit_report_path:
            ensure_dir_within_base(str(Path(args.launch_authorization_chain_of_chain_approval_token_audit_report_path).parent))
        if args.launch_authorization_super_chain_report_path:
            ensure_dir_within_base(str(Path(args.launch_authorization_super_chain_report_path).parent))
        if args.launch_authorization_super_chain_freshness_summary_path:
            ensure_dir_within_base(str(Path(args.launch_authorization_super_chain_freshness_summary_path).parent))
        if args.launch_authorization_super_chain_approval_token_path:
            ensure_dir_within_base(str(Path(args.launch_authorization_super_chain_approval_token_path).parent))
        if args.launch_authorization_super_chain_approval_token_revocation_log_jsonl_path:
            ensure_dir_within_base(str(Path(args.launch_authorization_super_chain_approval_token_revocation_log_jsonl_path).parent))
        if args.launch_authorization_chain_of_chain_approval_token_revocation_log_jsonl_path:
            ensure_dir_within_base(str(Path(args.launch_authorization_chain_of_chain_approval_token_revocation_log_jsonl_path).parent))
    ...
    if bool(args.launch_authorization_packet_approval_token_revoke_now):
        ...
        if bool(args.launch_authorization_packet_approval_token_revoke_only):
            return 0
    if bool(args.launch_authorization_chain_approval_token_revoke_now):
        chain_token_for_revoke = _read_json_or_empty(str(args.launch_authorization_chain_approval_token_path or ""))
        revoked_chain_token_row = revoke_live_pilot_launch_authorization_chain_approval_token(
            revocation_log_jsonl_path=str(args.launch_authorization_chain_approval_token_revocation_log_jsonl_path or ""),
            approval_token=chain_token_for_revoke,
            operator_id=str(args.operator_decision_actor or args.handoff_operator_id or ""),
            reason=str(args.launch_authorization_chain_approval_token_revoke_reason or "manual_revoke"),
        )
        print(json.dumps({"launch_authorization_chain_approval_token_revoked": revoked_chain_token_row}, sort_keys=True))
        if bool(args.launch_authorization_chain_approval_token_revoke_only):
            return 0
    if bool(args.launch_authorization_chain_of_chain_approval_token_revoke_now):
        chain2_token_for_revoke = _read_json_or_empty(str(args.launch_authorization_chain_of_chain_approval_token_path or ""))
        revoked_chain2_token_row = revoke_live_pilot_launch_authorization_chain_of_chain_approval_token(
            revocation_log_jsonl_path=str(args.launch_authorization_chain_of_chain_approval_token_revocation_log_jsonl_path or ""),
            approval_token=chain2_token_for_revoke,
            operator_id=str(args.operator_decision_actor or args.handoff_operator_id or ""),
            reason=str(args.launch_authorization_chain_of_chain_approval_token_revoke_reason or "manual_revoke"),
        )
        print(json.dumps({"launch_authorization_chain_of_chain_approval_token_revoked": revoked_chain2_token_row}, sort_keys=True))
        if bool(args.launch_authorization_chain_of_chain_approval_token_revoke_only):
            return 0
    if bool(args.launch_authorization_super_chain_approval_token_revoke_now):
        super_chain_token_for_revoke = _read_json_or_empty(str(args.launch_authorization_super_chain_approval_token_path or ""))
        revoked_super_chain_token_row = revoke_live_pilot_launch_authorization_super_chain_approval_token(
            revocation_log_jsonl_path=str(args.launch_authorization_super_chain_approval_token_revocation_log_jsonl_path or ""),
            approval_token=super_chain_token_for_revoke,
            operator_id=str(args.operator_decision_actor or args.handoff_operator_id or ""),
            reason=str(args.launch_authorization_super_chain_approval_token_revoke_reason or "manual_revoke"),
        )
        print(json.dumps({"launch_authorization_super_chain_approval_token_revoked": revoked_super_chain_token_row}, sort_keys=True))
        if bool(args.launch_authorization_super_chain_approval_token_revoke_only):
            return 0
    if bool(args.live_launch_guard_enforce):
        ...
        launch_authorization_chain_of_chain_approval_token_obj = _read_json_or_empty(
            str(args.launch_authorization_chain_of_chain_approval_token_path or "")
        )
        launch_authorization_super_chain_report_obj = _read_json_or_empty(str(args.launch_authorization_super_chain_report_path or ""))
        launch_authorization_super_chain_approval_token_obj = _read_json_or_empty(
            str(args.launch_authorization_super_chain_approval_token_path or "")
        )
        ...
        revoked_chain_auth_token_rows = list_live_pilot_launch_authorization_chain_approval_token_revocations(
            str(args.launch_authorization_chain_approval_token_revocation_log_jsonl_path or "")
        )
        revoked_chain2_auth_token_rows = list_live_pilot_launch_authorization_chain_of_chain_approval_token_revocations(
            str(args.launch_authorization_chain_of_chain_approval_token_revocation_log_jsonl_path or "")
        )
        revoked_super_chain_auth_token_rows = list_live_pilot_launch_authorization_super_chain_approval_token_revocations(
            str(args.launch_authorization_super_chain_approval_token_revocation_log_jsonl_path or "")
        )
        live_guard = evaluate_live_launch_guard(
            ...
            launch_authorization_chain_of_chain_approval_token=launch_authorization_chain_of_chain_approval_token_obj,
            launch_authorization_super_chain_report=launch_authorization_super_chain_report_obj,
            launch_authorization_super_chain_approval_token=launch_authorization_super_chain_approval_token_obj,
            ...
            require_launch_authorization_chain_report=bool(args.live_launch_guard_require_authorization_chain_report),
            require_launch_authorization_chain_approval_token=bool(args.live_launch_guard_require_authorization_chain_approval_token),
            ...
            require_launch_authorization_chain_of_chain_report_binding=bool(
                args.live_launch_guard_require_authorization_chain_of_chain_report_binding
            ),
            require_launch_authorization_chain_of_chain_approval_token=bool(
                args.live_launch_guard_require_authorization_chain_of_chain_approval_token
            ),
            require_unrevoked_launch_authorization_chain_of_chain_approval_token=bool(
            ...
            require_launch_authorization_super_chain_approval_token=bool(
                args.live_launch_guard_require_authorization_super_chain_approval_token
            ),
            require_unrevoked_launch_authorization_super_chain_approval_token=bool(
                args.live_launch_guard_require_unrevoked_authorization_super_chain_approval_token
            ),
            ...
            required_launch_authorization_chain_approval_action=str(args.live_launch_guard_authorization_chain_approval_action or "approve_live_launch_chain"),
            ...
            max_launch_authorization_chain_of_chain_approval_token_age_seconds=float(
                args.live_launch_guard_max_authorization_chain_of_chain_approval_token_age_seconds or 900.0
            ),
            max_launch_authorization_super_chain_report_age_seconds=float(
                args.live_launch_guard_max_authorization_super_chain_report_age_seconds or 900.0
            ),
            max_launch_authorization_super_chain_approval_token_age_seconds=float(
                args.live_launch_guard_max_authorization_super_chain_approval_token_age_seconds or 900.0
            ),
            ...
            revoked_launch_authorization_chain_approval_tokens=revoked_chain_auth_token_rows,
            revoked_launch_authorization_chain_of_chain_approval_tokens=revoked_chain2_auth_token_rows,
            revoked_launch_authorization_super_chain_approval_tokens=revoked_super_chain_auth_token_rows,
        )
        ...
        if str(args.launch_authorization_chain_report_path or "").strip():
            chain_report_out = build_live_pilot_launch_authorization_chain_report(
                launch_authorization_packet=(launch_authorization_packet_out or launch_authorization_packet_obj),
            ...
            chain_report_out = {}
        if str(args.launch_authorization_chain_approval_token_path or "").strip():
            write_live_pilot_launch_authorization_chain_approval_token(
                build_live_pilot_launch_authorization_chain_approval_token(
                    ...
                    operator_id=str(args.operator_decision_actor or args.handoff_operator_id or "main_user"),
                    approval_action=str(args.launch_authorization_chain_approval_action or "approve_live_launch_chain"),
                    expires_in_seconds=float(args.launch_authorization_chain_approval_expires_seconds or 900.0),
                ),
        ...
        if str(args.launch_authorization_chain_of_chain_freshness_summary_path or "").strip():
            ...
            chain_of_chain_freshness_summary = {}
        if str(args.launch_authorization_chain_of_chain_approval_token_path or "").strip():
            write_live_pilot_launch_authorization_chain_of_chain_approval_token(
                build_live_pilot_launch_authorization_chain_of_chain_approval_token(
                    launch_authorization_chain_of_chain_report=(chain_of_chain_report or launch_authorization_chain_of_chain_report_obj),
                    operator_id=str(args.operator_decision_actor or args.handoff_operator_id or "main_user"),
                    approval_action=str(args.launch_authorization_chain_of_chain_approval_action or "approve_live_launch_chain_of_chain"),
                    expires_in_seconds=float(args.launch_authorization_chain_of_chain_approval_expires_seconds or 900.0),
                ),
                str(args.launch_authorization_chain_of_chain_approval_token_path),
            )
        chain_of_chain_approval_token_obj_effective = (
            _read_json_or_empty(str(args.launch_authorization_chain_of_chain_approval_token_path or ""))
            or launch_authorization_chain_of_chain_approval_token_obj
        )
        if str(args.launch_authorization_chain_of_chain_approval_token_audit_report_path or "").strip():
            chain_of_chain_approval_token_audit_report = build_live_pilot_launch_authorization_chain_of_chain_approval_token_audit_summary(
                approval_token=chain_of_chain_approval_token_obj_effective,
                revoked_approval_tokens=revoked_chain2_auth_token_rows,
            )
            write_live_pilot_launch_authorization_chain_of_chain_approval_token_audit_summary(
                chain_of_chain_approval_token_audit_report,
                str(args.launch_authorization_chain_of_chain_approval_token_audit_report_path),
            )
        else:
            chain_of_chain_approval_token_audit_report = {}
        if str(args.launch_authorization_super_chain_report_path or "").strip():
            super_chain_report = build_live_pilot_launch_authorization_super_chain_report(
                launch_authorization_chain_of_chain_report=(chain_of_chain_report or launch_authorization_chain_of_chain_report_obj),
                launch_authorization_chain_of_chain_approval_token=chain_of_chain_approval_token_obj_effective,
                launch_authorization_chain_of_chain_approval_token_audit_summary=chain_of_chain_approval_token_audit_report,
                live_launch_guard_report=live_guard,
            )
            write_live_pilot_launch_authorization_super_chain_report(
                super_chain_report,
                str(args.launch_authorization_super_chain_report_path),
            )
        else:
            super_chain_report = {}
        if str(args.launch_authorization_super_chain_approval_token_path or "").strip():
            write_live_pilot_launch_authorization_super_chain_approval_token(
                build_live_pilot_launch_authorization_super_chain_approval_token(
                    launch_authorization_super_chain_report=(super_chain_report or launch_authorization_super_chain_report_obj),
                    operator_id=str(args.operator_decision_actor or args.handoff_operator_id or "main_user"),
                    approval_action=str(args.launch_authorization_super_chain_approval_action or "approve_live_launch_super_chain"),
                    expires_in_seconds=float(args.launch_authorization_super_chain_approval_expires_seconds or 900.0),
                ),
                str(args.launch_authorization_super_chain_approval_token_path),
            )
        if str(args.launch_authorization_super_chain_freshness_summary_path or "").strip():
            super_chain_freshness_summary = build_live_pilot_launch_authorization_super_chain_freshness_summary(
                launch_authorization_super_chain_report=(super_chain_report or launch_authorization_super_chain_report_obj),
                launch_authorization_chain_of_chain_freshness_summary=chain_of_chain_freshness_summary,
                max_super_chain_report_age_seconds=float(args.live_launch_guard_max_authorization_super_chain_report_age_seconds or 900.0),
            )
            write_live_pilot_launch_authorization_super_chain_freshness_summary(
                super_chain_freshness_summary,
                str(args.launch_authorization_super_chain_freshness_summary_path),
            )
